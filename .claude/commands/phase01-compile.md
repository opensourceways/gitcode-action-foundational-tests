...
| 2 | `permissions.hook` 范围 | 仅支持 `repository` `pr` `issue` `note` `project` |
| 3 | `permissions.actions/checks/contents/deployments/packages` 等 | 同上，这些范围不存在 |
| 4 | 顶层 `post:` 属性块（含 `run_always`、`steps`） | 移除，或通过 job 内步骤实现 |
| 5 | `runs-on` 中使用 `${{ }}` 表达式 | 使用硬编码 flow sequence 数组 |
| 6 | `${...}` 不支持的变量引用语法 | 必须使用 `${{ }}` 表达式语法 |
| 7 | `if:` 中裸 `always` 关键字（不写 `()`） | 必须写为 `always()` 函数调用 |
| 8 | `if:` 中 `$${ expr }` 语法（双美元） | 必须使用 `${{ expr }}` |
| 9 | `workflow_call.inputs` key 含连字符 `-` | 只能用字母、数字、下划线 |
| 10 | job/step 的 `env` key 含连字符 `-` | 只能用字母、数字、下划线 |
| 11 | `on.push` / `merge_requests` 同时存在 `paths` 和 `paths-ignore` | 保留 `paths`，删除 `paths-ignore` |
| 12 | `runs-on` 单字符串非 `default`/`ubuntu-latest`/`euler-latest` | 使用 flow sequence 数组 |
| 13 | 未包裹 `${{ }}` 的 `if:` 表达式 | 必须加 `${{ }}` 包裹 |
