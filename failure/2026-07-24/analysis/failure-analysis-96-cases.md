...
> 逐例详情: `failure/2026-07-24/case/*.md`（96 例，每例含全量日志 + 规格行号引用 + YAML 映射 + 失败传导链 + 影响评估）
...
| 指标 | 🔴严重 | 🟡中等 | ⚪/🟢 无影响 |
|---|---|---|---|
| **阻塞性** | 16 (17%) 平台阻断 | 22 (23%) 非阻塞但行为不符 | 58 (60%) 用例侧问题 |
| **静默性** | 16 (17%) 静默错误 | 41 (42%) 可察觉但缺诊断 | 39 (41%) 明确报错 |
| **影响面** | 7 (7%) 跨维度 | 26 (27%) 同维度 | 63 (66%) 单用例 |

### 3.2 核心发现：非阻塞静默错误构成最大风险

**17%（16 条）的失败是「非阻塞 + 静默错误」组合**——平台不报错但行为与文档不符，用户完全无法察觉：

| 用例 | 问题 | 用户后果 |
|---|---|---|
| USE-CONC-01-001 | `concurrency.max:10` 静默接受 | 用户配置超范围并发，行为不可预期 |
| USE-CTX-01-001 | `atomgit.ref` 返回 `main` 而非 `refs/heads/main` | 按文档写 `cut -d'/' -f3` 提取分支名的脚本静默失败 |
| USE-CTX-01-002 | `github.ref` 被静默求值为占位符 | 迁移用户引用 `github.*` 得到无效值，无任何提示 |
| USE-EXPR-01-001 | 未知属性求值为空串 | 用户拼写错误 `atomgit.xxx` 被静默忽略，调试困难 |
| USE-INPT-01-002 | `type:boolean` 静默降级 | 用户用 boolean input 得到 string，条件判断逻辑出错 |
| USE-OS-01-001 | `runner.os` 返回 `linux`（小写）vs 文档 `Linux` | 用户 `if: runner.os == 'Linux'` 永远为 false |
| USE-SECNAME-01-001 | `ATOMGIT_` 前缀 secret 被接受 | 命名违规被静默允许，未来系统变量冲突时静默出错 |
| REL-YAMLCACHE-01-060 | YAML 修改后缓存未失效，执行旧版代码 | 用户推送修复后平台仍执行旧版恶意/错误 workflow |
| REL-FAULT-01-031 | SIGKILL 故障注入未工作 | runner 异常终止后的日志保留/恢复功能未验证 |
| REL-FAULT-01-032 | 网络分区故障注入未工作 | 网络中断时的 artifact 重试/恢复行为未验证 |
| REL-FAULT-01-033 | 磁盘满故障注入未工作 | 磁盘耗尽时的 job 优雅降级行为未验证 |
| COMPAT-DIR-01-002 | `.github/workflows/` 被错误触发 | 迁移用户的旧 workflow 被意外执行，安全边界破坏 |
| COMPAT-CACHE-01-001 | cache 在 dispatch 事件静默 MISS | 用户手动触发 CI 时 cache 从不命中，构建速度无改善且无警告 |
| SEC-DOS-01-001 | 1.1GB 制品静默上传成功 | DoS 攻击者可无限制上传大制品耗尽存储配额 |
| SEC-MASK-01-001 | secret 输出为空而非 `***` | 用户无法确认脱敏是否生效，无法区分"脱敏了"与"secret 未配置" |
...
| 维度 | 阻塞 | 非阻塞 | 无影响 | 核心风险 |
|---|---|---|---|---|
| completeness | 11 | 1 | 4 | artifact/cache/call 基础功能完全阻塞，P0 阻断 |
| compatibility | 2 | 3 | 19 | 大量标记不匹配，3 个 Engine Bug（大小写） |
| reliability | 4 | 12 | 8 | harness 超时覆盖率极高，故障注入全失效 |
| security | 4 | 10 | 9 | 真实安全缺陷少（DOS/MASK/SUPPLY），多为标记不匹配 |
| usability | 0 | 7 | 2 | **全为非阻塞静默错误**——每个都可能导致用户脚本静默失败 |
...
27 条无法规避的失败中：
- 11 条 artifact/cache/call 基础功能阻塞（需平台修复）
- 4 条 harness 超时覆盖（需增加 harness timeout 或平台 timeout 缩短）
- 4 条 0 字节有效日志（需平台增加诊断输出）
- 3 条 故障注入不工作（需平台支持故障注入）
- 5 条 其他
...
### 4.1 run_status 词汇映射缺失——第一大断裂（40 条，42%）
...
### 4.3 `${{{{ }}}} ` 四括号——合约生成缺陷（4 条 Reliability）
