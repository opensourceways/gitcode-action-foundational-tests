...
const RUN_DIR = 'phase01/runs/2026-07-21-02';
const TEXT_DIR = path.join(RUN_DIR, 'cases', 'text');
const YAML_DIR = path.join(RUN_DIR, 'cases', 'yaml');

function ensureDir(dir) {
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
}

function writeTextCase(c) {
  const filePath = path.join(TEXT_DIR, `${c.id}.md`);
  const motherLine = c.mother ? `母意图:    ${c.mother}` : '母意图:    —';
  const content = `用例 ID:   ${c.id}
维度标签:   ${c.dimensions}
维度:      ${c.dimension_label}
优先级:    ${c.priority}
溯源意图:  ${c.intent_ref}
${motherLine}
标题:      ${c.title}
...
验证点:
${c.verification_points.map(v => `  - ${v}`).join('\n')}

清理:      ${c.cleanup}
`;
  fs.writeFileSync(filePath, content, 'utf-8');
  console.log(`[text] ${c.id}`);
}

function writeYamlCase(c) {
  const filePath = path.join(YAML_DIR, `${c.id}.yaml`);
  const workflowBlock = c.workflow;
  const triggerParamsYaml = Object.entries(c.trigger_params || {})
    .map(([k, v]) => `    ${k}: "${v}"`).join('\n');

  const assertionsYaml = c.assertions.map(a => {
    let lines = `  - type: ${a.type}\n    target: ${a.target}`;
  ...
  const faultYaml = c.fault_injection
    ? `\n  at: ${c.fault_injection.at}\n  action: ${c.fault_injection.action}\n  params:\n${Object.entries(c.fault_injection.params || {}).map(([k, v]) => `    ${k}: ${v}`).join('\n')}\n  recovery_expectation: ${c.fault_injection.recovery_expectation}`
    : 'null';

  const secretsYaml = c.secrets && c.secrets.length
    ? `[${c.secrets.map(s => `"${s}"`).join(', ')}]`
    : '[]';

  const content = `id: ${c.id}
...
trigger:
  ...
  as: ${c.trigger_as}
  params:
${triggerParamsYaml ? triggerParamsYaml + '\n' : ''}
fault_injection: ${faultYaml}
...
`;
  fs.writeFileSync(filePath, content, 'utf-8');
  console.log(`[yaml] ${c.id}`);
}

const cases = [];

// ===== SEC batch (14 cases: 12 independent + 2 variants) =====
...
console.log(`Done. Generated ${cases.length} cases.`);
