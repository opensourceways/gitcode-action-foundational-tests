...
## 9b. 全集原则 ★（2026-07-20 生效）

- **Phase 01 产出的是测试用例全集，不是补集。** `/phase01-gen` 产出的文本用例 + 可执行 YAML 就是最终交付给 Phase 02 的**唯一用例集**。
- **已有用例是输入素材，不是平行体系。** `inputs/existing-cases/` 中的历史用例由 agent 评估：高价值的吸收进 Phase 01 用例（用 Phase 01 格式重表达，保留溯源标记），作用有限/冗余/已过时的显式标注 `superseded` 并淘汰。交付给 Phase 02 时不存在「新用例 + 旧用例」两套清单。
- **覆盖度即全集覆盖度。** `coverage.md` 和 `dod-checklist.md` 只以 Phase 01 产出的用例为统计口径。被淘汰的旧用例不计入覆盖。
- **淘汰标准**：满足以下任一可淘汰：(a) 纯文档抄录型，无独立验证价值（如「文档说 X 字段存在，跑一遍确认」）；(b) 已被新用例的行为级断言完全覆盖；(c) 标 `D 测不动` 且当前环境仍不可测；(d) 标 `SKIP` 且 skip 原因长期有效。
- **淘汰留痕**：被淘汰的用例在 `case-manifest.md` 中保留一行记录，标注 `superseded-by <Phase01-ID>` 或 `deprecated: <原因>`，确保回溯链不断。
