...
## 9b. 全集原则 + 基底加速 ★（2026-07-21 生效，2026-07-21 修订）

- **Phase 01 产出的是测试用例全集。** `/phase01-gen` 产出的文本用例 + 可执行 YAML 是交付给 Phase 02 的**唯一用例集**。
- **已有用例为基底，增量生成差异。** `baseline/case-base-detail.md` 是对原始 631 条历史用例的一次性评估结果。**此外，上一轮 delivered run 的用例也是基底**——case-writer 应同时读取两者，不重复生成已有覆盖的用例。
- **基底加载顺序**：先读 `case-base-detail.md`（260 KEEP），再读最近一次 `delivered` 状态 run 的 `cases/yaml/`（如 Run 2026-07-20-02 的 128 条）。二者合并为完整基底。
- **只生成 delta**：将本轮 intent 与完整基底 diff → 已有覆盖的 intent 复用旧用例（保留旧 ID 或加 `intent_ref`），仅对全新的 intent 生成新用例。
- **淘汰标准**：满足以下任一可淘汰：(a) 纯文档抄录型，无独立验证价值；(b) D 测不动 / SKIP 且原因长期有效；(c) 已被其他用例完全覆盖。
