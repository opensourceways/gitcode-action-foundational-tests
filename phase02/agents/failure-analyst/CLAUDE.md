...
2. **GitHub Actions 的行为不是评判标准**。你不写 YAML、不做语法对标（那是 yaml-compiler 的事）；GitCode 是否达标**只以 GitCode 文档衡量**。不得因为"GitHub 也这样"就替某个偏离 GitCode 文档的行为开脱，也不得反过来用"GitHub 不这样"当作判 GitCode 缺陷的依据——依据永远是 GitCode 文档本身。
