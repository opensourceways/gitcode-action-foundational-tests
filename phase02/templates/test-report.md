...
> 以下为本批次执行的全部用例清单，由 report-builder 从 `cases/yaml/*.yaml` 逐条提取填充。每条用例标注**测试维度**、**是否自动化**及**执行结果**。

| 序号 | 用例 ID | 标题 | 测试维度 | 是否自动化 | 测试结果 |
|:---:|---|---|---|:---:|---|
| <index> | <case-id> | <title> | <dimension> | <automated> | <result> |
...
| **是否自动化** | ✅ 自动化（assertion 可完全由 harness 判定）<br>⚠️ 半自动化（含 `eval: llm_assisted` 需 LLM 辅助评判）<br>❌ 非自动化（需人工验收，如 UI 可读性、文档一致性巡检） |
| **测试结果** | ✅ 通过 / ❌ 未通过 / ⏳ 待确认（Flaky 或人工未判） |
